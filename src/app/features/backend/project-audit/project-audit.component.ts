...
import { AuditLogEntry } from '../../../core/models/backend.model';
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <h2 class="text-lg font-medium text-zinc-200 flex items-center gap-2">
          <lucide-icon [img]="ScrollText" [size]="18" />
          Audit Log
        </h2>
      </div>
...
export class ProjectAuditComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (projectChanged || connectionChanged) {
      this.offset.set(0);
      this.page.set(1);
      this.actionFilter = '';
      this.loadLogs();
  ...
  async loadLogs(): Promise<void> {
    if (!this.projectId || !this.connectionId) {
      this.entries.set([]);
      return;
    }
    ...
    try {
      const entries = await this.backend.getAuditLogsFor(this.connectionId, this.projectId, {
        limit: this.pageSize,
        offset: this.offset(),
